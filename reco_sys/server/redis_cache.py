...
def get_reco_from_cache(temp, hbu):
    ...
    res = redis_client.zrevrange(key, 0, temp.article_num - 1)
    # - 如果redis有，读取需要推荐的文章数量放回，并删除这些文章，并且放入推荐历史推荐结果中
    if res:
        redis_client.zrem(key, *res)
    else:
        ...
        if len(hbase_cache) > 100:
            ...
            # 放入redis缓存
            redis_client.zadd(key, dict(zip(cache_redis, range(len(cache_redis)))))
            # 剩下的放回wait hbase结果
            ...
            # 放入redis缓存
            redis_client.zadd(key, dict(zip(hbase_cache, range(len(hbase_cache)))))
